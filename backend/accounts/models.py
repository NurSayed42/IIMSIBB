...
class Inspection(models.Model):
    ...
    # Section L: Site Photos & Video
    site_photos = models.JSONField(default=list, blank=True, null=True)
    site_video = models.JSONField(default=dict, blank=True, null=True)
    ...
    def get_location_summary(self):
        """Get a summary of location tracking data"""
        location_points = self._parse_json_field(self.location_points)
        ...
        return f"{len(location_points)} points | {start_time} to {end_time}"

    def get_first_location(self):
        """Get the first location point - FIXED"""
        location_points = self._parse_json_field(self.location_points)
        if location_points and len(location_points) > 0:
            first_point = location_points[0]
    ...
    def get_last_location(self):
        """Get the last location point - FIXED"""
        location_points = self._parse_json_field(self.location_points)
        if location_points and len(location_points) > 0:
            last_point = location_points[-1]
    ...
    def _parse_json_field(self, field_data):
        """Helper method to safely parse JSON fields - IMPROVED VERSION"""
        if not field_data:
            return []
        ...
        if isinstance(field_data, str):
            try:
                ...
                # Ensure we return a list if it's a single dictionary
                if isinstance(parsed, dict):
                    return [parsed]
                return parsed
            except json.JSONDecodeError:
                print(f"❌ JSON decode error for field: {field_data}")
                # If it's a string but not JSON, return as single item list
            ...
            print(f"⚠️ Unknown field type: {type(field_data)} - {field_data}")
            return []
