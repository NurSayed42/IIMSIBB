    ...
    try {
      ...
      if (data) {
        const parsedData = {
          ...
          site_video: parseJsonField(data.site_video),// ✅ এই লাইনটি add করুন
    ...
    if (media.base64_data) {
      if (media.base64_data.startsWith('data:')) {
        return media.base64_data;
                  ...
                  // newly added 
        if (media.type === 'video' || media.name?.includes('.mp4') || media.file_name?.includes('.mp4')) {
        return `data:video/mp4;base64,${media.base64_data}`;
        } else {
          // close added 
...
// ✅ ADD HERE: Video Thumbnail Rendering Function
const renderVideoThumbnail = (video, index) => {
  const videoUrl = getMediaUrl(video);
  const videoName = video.name || `Video ${index + 1}`;
  
  console.log(`Rendering video ${index}:`, { videoUrl, videoName });

  return (
    <div 
      key={index}
      className="border border-gray-300 rounded-lg overflow-hidden bg-gray-100 hover:shadow-lg transition-shadow"
    >
        ...
        {videoUrl ? (
          <>
            <video 
              ...
              poster="https://via.placeholder.com/300x200/4F46E5/FFFFFF?text=Video+Preview"
              controls
              preload="metadata"
            >
          ...
          <div className="w-full h-48 flex flex-col items-center justify-center bg-gray-200">
            <span className="text-4xl mb-2">📹</span>
            <span className="text-sm">{videoName}</span>
            <span className="text-xs text-red-500 mt-1">Video URL not available</span>
          </div>
        )}
      </div>
      
      <div className="p-3 bg-white">
        <div className="font-medium text-gray-800 truncate">{videoName}</div>
...
// Enhanced Location Tracking Section with Google Maps
  ...
  const renderMediaSection = () => {
    ...
    console.log('Media section data:', { 
      sitePhotos: sitePhotos.length, 
      siteVideo: siteVideo.length, 
    ...
    return (
      <div className="mb-6">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {/* ✅ ADD THIS: Video Section */}
          {siteVideo.length > 0 && (
            <div className="mb-6">
              <h4 className="font-semibold text-green-800 mb-3">Site Videos</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {siteVideo.map((video, index) => 
